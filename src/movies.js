...
const ulList = document.querySelector(".movies-list");
const ulMost = document.querySelector(".movies-most");
const ulLeast = document.querySelector(".movies-least");

//--functions---------------------------------------
function getPosterImages() {
	arImages =  moviesData.results[moviesData.results.length - 1].map(movie =>
	...
	//--operations----------------------------------
	ulList.innerHTML = getPosterImages();
	ulMost.innerHTML = getPosterImages(); //здесь нужно выбрать лучший
	ulLeast.innerHTML = getPosterImages(); 